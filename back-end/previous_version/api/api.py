...
def login():
    if not request.is_json: 
        ...
        data = request.get_json()  # ✅ Correct way to handle JSON requests
        if not data:
...
def logout():
    ...
    if user:
        del sessions[user]
        return jsonify({"message": "Logout successful"}), 200  # ✅ Always return JSON

    return jsonify({"error": "Invalid session"}), 401  # ✅ Already correct
...
def get_tolls():
    try:
        ...
        return jsonify({"error": "Failed to fetch tolls", "details": str(e)}), 500
...
def count_operator_passes(operatorID):
    try:
        ...
        # Query to count passes for all tolls under this operator
        query = """
        ...
        if result and result["totalPasses"] is not None:
            ...
            return jsonify({"operatorID": operatorID, "totalPasses": 0}), 200  # No passes found

    except Exception as e:
        return jsonify({"error": "Failed to count toll passes", "details": str(e)}), 500
...
def get_operator_debts(operatorID):
    try:
        ...
        return jsonify({"error": "Failed to fetch operator debts", "details": str(e)}), 500
...
def get_operators():
    try:
        ...
        return jsonify({"error": "Failed to fetch operators", "details": str(e)}), 500
